/*-----------------------------------------------------------------------------
*作者:jane, email:316512679@qq.com
*version:1.0  , 
...
myLib.desktop.win={
	winHtml:function(title,url,id){
		...
		+title
		+"</b><span class='win_btnblock'><a class='winMinimize' title='最小化'></a><a class='winMaximize' title='最大化'></a><a class='winHyimize' title='还原'></a><a  class='winClose' title='关闭'></a></span></div><iframe frameborder='0' name='myFrame_"
		+id
		+"' id='myFrame_"
		+id
	...
	maximizeWin:function(obj){
		var myData=myLib.desktop.getMydata(),
		    ...
		    wh=myData.winWh;//获取当前document宽高
		obj
		.css({'width':wh['w'],'height':wh['h'],'left':-panel.left,'top':-panel.top,'visibility':'visible'})
		.draggable( "disable" )
		.resizable( "disable" )
		//.fadeTo("fast",1)
		.find(".winframe")
    ...
    newWin:function(options){
		...
		var wLeft=(wh['w']-options['WindowWidth'])/2-panelLeft.left-dxy;
		var wTop=(wh['h']-options['WindowHeight'])/2;
		...
		winMinimize_btn.click(function(){
						...
						$("#desktopPanel").css("z-index",70);						
						 ...
						 var winsLeft=wins.position().left;
						 if(wins.css('visibility')=='hidden'){
							  //wins.show();
							  wins.css({'left':winsLeft+10000,'visibility':'visible'}).removeClass("hideWin");
							  objTab.removeClass('defaultTab').addClass('selectTab');//当只有一个窗口时
						      myLib.desktop.win.switchZindex(wins);
							 }else{			  
								//wins.css({"left":winsLeft-10000,"visibility":"hidden"});
...
myLib.desktop.lrBar={
	...
	init:function(){
		...
		.click(function(){
						var i = $(this).attr('p');
						myLib.desktop.deskIcon.desktopMove($innerPanel,$deskIcon,$def_tab,500,desktopWidth+lBarWidth,i);
						})
		// .draggable({
				// helper: "clone",
				// scroll:false,
	...
	contextMenu:function(tab,id){
		var _this=this;
		 ...
		 var data=[
					...
					text:"最大化",
					func:function(){
						var wins=$("#myWin_"+tab.data('win'));
						var winLeft=wins.position().left;
						if(wins.find('a.winMaximize').is(':hidden')){
							wins.css({'left':winLeft+10000,'visibility':'visible','z-index':100});
						}else{
							wins.find('a.winMaximize').trigger('click');
							}
						myLib.desktop.win.switchZindex(wins);
						}
					  },{
		...
		var myData=myLib.desktop.getMydata();
 		var $task_lb=myData.panel.taskBar['task_lb']
			...
			if(curTabNum>myData.maxHdTabNum-1){
				...
				//$task_lb.parent().css("margin","0 100px");
				}
...
myLib.desktop.deskIcon={
	...
	upDesktop:function($deskIcon,$deskIconBlock,$innerPanel,$deskIconNum,lBarWidth){
		 var myData=myLib.desktop.getMydata()
		    ,winWh=myData.winWh
		    ,w=(winWh['w']-lBarWidth)
		    ,h=(winWh['h']-75)
		...
		//设置桌面图标容器元素区域大小
		 ...
		 $deskIcon.css({"width":w+"px","height":h+"px",'margin-right':lBarWidth+'px'});
		 $deskIconBlock.css({"width":w+"px","height":h+"px",'margin-left':lBarWidth+'px','margin-bottom':75+"px"});
	...
	init:function(iconData){
		 ...
		 _this.upDesktop($deskIcon,$deskIconBlock,$innerPanel,$deskIconNum,lBarWidth);
		 //如果窗口大小改变，则重新排列图标
		 $(window).wresize(function(){
							myLib.desktop.winWH();//更新窗口大小数据
							$('.currDesk').trigger('click');
							_this.upDesktop($deskIcon,$deskIconBlock,$innerPanel,$deskIconNum,lBarWidth);
   								   });
 		 //附加data数据
		 myLib.desktop.iconDataInit(iconData);
		 ...
		 // .draggable({
					...
					// stop:function(event,ui){
						...
						// if(dxCha < -150 && deskIndex<3){
 						// _this.desktopMove($(this),$deskIcon,dates,moveDx,deskIndex+1);
						// 右移	 
